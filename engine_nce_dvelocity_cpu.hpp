...
template<uint8_t NC>
class engine_nce_dvelocity_cpu : public engine_base
{
...
public:
	...
	const static uint8_t N_VARS = NC + 1 + 1;  // +1 velocity (interface unknown) + enthalpy
	// number of variables per node
	const static uint8_t N_NODE = NC + 1;  // +1 velocity (interface unknown) + enthalpy
	...
	// number of operators:Mass eq: NC accumulation operators, NC flux operators 
	//Energy eq: fluid and rock energy accumulation, fluid energy flux, fluid temperature,  conduction, rock conduction, temperature
	// momentun equation: Reynold operator, Total density operator. 
	//const static uint8_t N_OPS = 2 * NC + 2;     //  + 2 ( total mobbility & total density ) 
	//const static uint8_t N_OPS = 2 * NC + 3 + 6;   //  + 2 ( total mobbility & total density & reynolds operatr )  + energy eq operators
	const static uint8_t N_OPS = 2 * NC + 6 + 3;   //  + 2 ( total mobbility & total density & reynolds operatr )  + energy eq operators
	// order of operators:
	/*
	  const static uint8_t ACC_OP = 0;
	const static uint8_t FLUX_OP = NC;
	const static uint8_t LAMBDA_OP = 2 * NC;
	const static uint8_t DENS_OP = 2 * NC ;
	const static uint8_t RE_OP = 2 * NC ;

	////
	const static uint8_t FE_ACC_OP = 2 * NC ;
	const static uint8_t RE_ACC_OP = 2 * NC  ;
	const static uint8_t FE_FLUX_OP = 2 * NC ;
	const static uint8_t FE_COND_OP = 2 * NC ;
	const static uint8_t RE_COND_OP = 2 * NC ;
	const static uint8_t FE_TEMP_OP = 2 * NC ;
	*/
	...
	const static uint8_t FLUX_OP = NC;
	const static uint8_t FE_ACC_OP = 2 * NC;
	const static uint8_t RE_ACC_OP = 2 * NC + 1;
	const static uint8_t FE_FLUX_OP = 2 * NC + 2;
	const static uint8_t FE_COND_OP = 2 * NC + 3;

	const static uint8_t RE_COND_OP = 2 * NC + 4;
	const static uint8_t FE_TEMP_OP = 2 * NC + 5;
	// additional momentum OP
	const static uint8_t LAMBDA_OP = 2 * NC + 6;
	const static uint8_t DENS_OP = 2 * NC + 7;
	const static uint8_t RE_OP = 2 * NC + 8;
	...
	const static uint8_t N_VARS_SQ = (N_VARS - 1) * (N_VARS - 1);
	...
	const int acc_active = (vel_linear) ? 0 : 0.;



	engine_nce_dvelocity_cpu() { engine_name = "Multiphase " + std::to_string(NC) + "- momentum in the wellbore is " + ("%s", vel_linear ? "Darcy" : "ms-well equation") + +"-component enthalpy-based thermal flow decoupled velocity CPU engine"; };
	...
	void momentum_mswell_segment_info(index_t iw, index_t j, value_t velocity, value_t p_diff, value_t dt, std::vector<value_t> &X, csr_matrix_base *jacobian, std::vector<value_t> &RHS, int assemble_kernel = 0);
	...
	void calculateFanningFactor_segment(value_t velocity, value_t roughness, value_t block_up, value_t &f, value_t &D);
	...
	value_t res_wel_vel;
	value_t res_res_vel;
	value_t conn_annulus = 0;
	//  take into acceleration in momentum equation or not.
	//  const static uint8_t acc_active = 0;
